...
public class Agent_Training : Agent
{
    ...
    public double[] currentMotion = { 0, 0 };
    //whether use Circular–Linear Flow Field map
    public bool useMoD;
    ...
    public override void Initialize()
    {
        ...
        if (this.useMoD)
        {
            this.modManager = GameObject.Find("MoD").GetComponent<ModManager>();
            this.mod = this.modManager.grid;
        }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        {
            this.initial = false;
            Vector3[] generatedPoints;
            //Get a random agent spawn point and rotate agent to look at target
            if (this.manager.spawnAsRealWorldData)
            {
                generatedPoints = GetSpawnPoints();
            }
            else
            {
    ...
    {
        Vector3[] ret = this.manager.GetStartEndPosition(this.agentID);
        Debug.Log(ret);
    ...
    {
        this.currentMotion[0] = AgentRb.velocity.magnitude;
        this.currentMotion[1] = Math.Atan2(transform.forward.z, transform.forward.x);
        ...
        //follow flow reward
        if (this.useMoD) {
            double motion_likelihood = this.mod.GetProb(transform.position, this.currentMotion);
            this.imitation_reward = this.imitationWeight * (float)motion_likelihood;
            AddReward(this.imitation_reward);
            //if (flow_angle > 0)
            //{
            //    float angle_radian = Mathf.PI * flow_angle / 180.0f;
            //    Vector3 flow_direction = new Vector3(Mathf.Cos(angle_radian), 0f, Mathf.Sin(angle_radian));
            //    // how much human follow flow (local path planning)
            //    float flow_face_angle = Vector3.Angle(transform.forward, flow_direction);
            //    // whether current follow contribute to goal (global path planning)
            //    float flow_goal_angle = Vector3.Angle(flow_direction, goalVector);
            //    //Moving towards goal reward
            //    if ((flow_face_angle <= 30f))
            //    {
            //        AddReward(+0.0005f * this.goalWeight);
            //    }
            //    else
            //        AddReward(-0.00025f * this.goalWeight);
            //}   
    ...
    private int getClosestAgent()
    {
        ...
        foreach (Transform child in this.agentParent)
        {
            ...
            //}
            if (dist > 0.01f) // check if the child is itself
            {
                this.closeAgents++;
                neighboursPoints.Add(child.position);
    ...
    private void getClosestInteraction()
    {
        ...
        // Debug.Log(this.closestInteraction);
    }
