...
public class Monitor_Training : MonoBehaviour
{
    ...
    [Tooltip("spawn and goal specific by real world data")]
    public bool spawnAsRealWorldData;
    [Tooltip("Real pedestrian start and goal position file")]
    public string spawn_goal_file;
    ...
    //spwan goal real world data buffer
    private SortedList<int, float[]> spawn_goal_buffer;
    private SortedList<double, List<int>> spawn_timing_buffer;
    ...
    void Start()
    {
        ...
        //Load real world data if need
        if (this.spawnAsRealWorldData)
        {
            LoadSpawnGoalData();
        }
        initializeGoalAreas();
    ...
    private void LoadSpawnGoalData()
    {
        StreamReader sr = new StreamReader(this.spawn_goal_file);
        this.spawn_goal_buffer = new SortedList<int, float[]>();
        this.spawn_timing_buffer = new SortedList<double, List<int>>();
        ...
        while ((currentLine = sr.ReadLine()) != null)
        {
            ...
            id = float.Parse(line[0]);
            spawn_timing = double.Parse(line[1]);
            spawn_x = float.Parse(line[2])/1000f;
            spawn_z = float.Parse(line[3]) / 1000f;
            goal_timing = float.Parse(line[4]);
            goal_x = float.Parse(line[5]) / 1000f;
            goal_z = float.Parse(line[6]) / 1000f;
            double timing = spawn_timing - 1352853188.71;
            
            // shift to zero min
            if (this.spawn_timing_buffer.ContainsKey(timing)){
                this.spawn_timing_buffer[timing].Add(agent_id);
            }
            else
            {
                this.spawn_timing_buffer.Add(timing, new List<int> { agent_id });
            }
            this.spawn_goal_buffer.Add(agent_id, new float[] { id, spawn_x, spawn_z, goal_x, goal_z });
            agent_id += 1;
        }
        this.numOfAgents = this.spawn_goal_buffer.Count();
    ...
    private IEnumerator instantiateAgents()
    {
        ...
        else
        {
            if (this.spawnAsRealWorldData)
            {
                ...
                foreach (KeyValuePair<double, List<int>> p in this.spawn_timing_buffer)
                {
                    // if (p.Key < previous_time) { continue; }
                    Debug.Log(p.Key - Time.time);
                    yield return new WaitForSeconds((float)(p.Key- Time.time));
                    foreach(int agent_id in p.Value)
                    {
                        GameObject tempAgent = Instantiate(this.agentPrefab, new Vector3(0, agent_id, 0), Quaternion.identity, this.agentParent.transform);
                        if (this.unlimitedEpisodeSteps)
                            tempAgent.GetComponent<Agent_Training>().MaxStep = 0;
                    }
    ...
    public Vector3[] GetStartEndPosition(int agent_id)
    {
        Vector3[] ret = new Vector3[2];
        //agent_id, new float[] { id, spawn_x, spawn_z, goal_x, goal_z }
        ret[0] = new Vector3(this.spawn_goal_buffer[agent_id][1], 0, this.spawn_goal_buffer[agent_id][2]);
        ret[1] = new Vector3(this.spawn_goal_buffer[agent_id][3], 0, this.spawn_goal_buffer[agent_id][4]);
    ...
    private void setWeights(float collision, float goal, float groub, float interact, bool random)
    {
        if (random)
        {
            this.collisionWeight = UnityEngine.Random.Range((this.collMin + this.collMax)/2.5f, this.collMax/1.25f);
            this.goalWeight = UnityEngine.Random.Range(this.goalMin, this.goalMax);
            this.groupWeight = UnityEngine.Random.Range(this.groupMin, this.groupMax);
            this.interactWeight = UnityEngine.Random.Range(this.interMin, this.interMax);
    ...
    private void setInteractionObjects()
    {
        ...
        int times = UnityEngine.Random.Range(3, len);
